...
import { tunings, fretStyles } from '../../utilities/service-logic';
...
export default function Fretboard() {
    ...
    <div className='fb-container flex'>
        <section className='tuner flex col'>
            {
                tunings[signature.tuning].map((note, idx) => <span className='note flex' key={idx}>{capitalize(note)}</span>)
            }
        </section> 
        <section className="Fretboard flex">
                {frets.map((group) => (
                    fretStyles[group].frets.map((f, idx) => <section className=' fret flex col' key={`${f}.${idx}`} style={{ width: fretStyles[group].width}}></section>)
                ))}
        </section>
    </div>)
}